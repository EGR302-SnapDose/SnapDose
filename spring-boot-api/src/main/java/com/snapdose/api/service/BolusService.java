...
public class BolusService {
    ...
    private static final Map<BolusStatus, Set<BolusStatus>> VALID_TRANSITIONS =
        Map.of(
            BolusStatus.PENDING,
            Set.of(
                BolusStatus.ACKNOWLEDGED,
                BolusStatus.CANCELLED,
                BolusStatus.FAILED
            ),
            BolusStatus.ACKNOWLEDGED,
            Set.of(
    ...
    public BolusResponse createBolus(BolusRequest request) throws Exception {
        ...
        return BolusResponse.pending(
            bolusId,
            request.getUnits(),
            request.getBolusType()
        );
    }

    public BolusResponse transitionStatus(
        String userId,
        String bolusId,
        BolusStatus newStatus,
        double unitsDelivered
    ) throws Exception {
        BolusRecord record = repository
            .findById(userId, bolusId)
            .orElseThrow(() ->
                new IllegalArgumentException("Bolus not found: " + bolusId)
            );

        Set<BolusStatus> allowed = VALID_TRANSITIONS.getOrDefault(
            record.getStatus(),
            Set.of()
        );
    ...
    public Optional<BolusResponse> getBolusStatus(String userId, String bolusId)
        throws Exception {
        return repository
            .findById(userId, bolusId)
            .map(record -> {
    ...
    public Optional<BolusRecord> getPendingBolus(String deviceId)
        throws Exception {
        return repository.findPendingByDeviceId(deviceId);
    }

    public void failStaleAcknowledgedBolus(String deviceId) throws Exception {
        repository
            .findAcknowledgedByDeviceId(deviceId)
            .ifPresent(record -> {
                try {
